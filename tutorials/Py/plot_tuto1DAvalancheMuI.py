...
Us = fluidfoam.readvector(sol, tread, 'U.a')
Uw = fluidfoam.readvector(sol, tread, 'U.b')
...
try:
    gradUs = fluidfoam.readtensor(sol, tread, 'grad(U.a)')
except:
...
print("max(Us)=" + str(np.amax(Us)) + " m/s")
...
ax2 = subplot(gs[0, 1])
l21, = ax2.plot(Us[0,:]/(g*d)**0.5, Y[:]/d, '-r')
l22, = ax2.plot(Uw[0,:]/(g*d)**0.5, Y[:]/d, '--b')
l22, = ax2.plot((alpha*Us[0,:]+(1-alpha)*Uw[0,:])/(g*d)**0.5, Y[:]/d, ':m')
l2, = ax2.plot(uex[:]/(g*d)**0.5, xex[:]/d, '--k')

ax2.set_xlabel(r'$U/\sqrt{g d}$')
ax2.set_xlim(0,  np.max([np.max(uex),np.max(Us[0,:])])/(g*d)**0.5 * 1.1)
