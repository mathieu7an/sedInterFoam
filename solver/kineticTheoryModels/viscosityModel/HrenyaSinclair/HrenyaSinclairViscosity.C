...
Foam::kineticTheoryModels::HrenyaSinclairViscosity::mus
(
...
) const
{
    ...
    (
        scalar(1) + ds/(6.0*sqrt(2.0)*(alphas + scalar(1.0e-5)))/L_
    );

    return rhos*ds*sqrt(Theta)*
    (
        (4.0/5.0)*sqr(alphas)*g0*(1.0 + e)/sqrtPi
      + (1.0/15.0)*sqrtPi*g0*(1.0 + e)*(3.0*e - 1)*sqr(alphas)/(3.0-e)
      + (1.0/6.0)*sqrtPi*alphas*(0.5*lamda + 0.25*(3.0*e - 1.0))
       /(0.5*(3.0 - e)*lamda)
...
) const
{
    ...
    return rhos*ds*sqrt(Theta)*
    (
        (4.0/3.0)*sqr(alphas)*g0*(1.0+e)/sqrtPi
    );
}
