...
{
    return Fr*pow(max(alphas_-alphasMinFriction, scalar(0)), eta0)*
            (1+sin(M_PI*(max(max(alphas_ - alphasMinFriction, scalar(0))/
                             (alphasMax- alphasMinFriction), scalar(0))-0.5)));
}
