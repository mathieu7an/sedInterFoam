...
{
    // MuI inertial regime friction model: mu(I) = muss + (mu2-muss)/(I0/I+1)
    //          with I=ds magD / sqrt(ps)
    return muss + (mu2 - muss)*magD/(I0*pow(ps, 0.5)/ds + magD + Dsmall);
}
