...
// Local references
    const volScalarField& alphaw = this->alpha_;
    const volScalarField alphas = 1 - alphaw;
    ...
    scalar alphasMaxLMs(alphasMaxLM_.value());
    scalar LmPhi(0.);
    scalar cmu34(pow(Cmu_.value(), 3.0/4.0));
    ...
    forAll(U, cellI)
    {
        ...
        LmPhi = LmPhi
              ...
              (
                  scalar(1.0)
                - Foam::pow(min(alphas[cellI]/alphasMaxLMs, scalar(1.0)),
                            expoLM), 0.
              )*dY;
        Lm = LmPhi;
